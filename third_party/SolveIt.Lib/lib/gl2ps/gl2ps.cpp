...
/* The glist handling routines */

void gl2psListRealloc(GL2PSlist *glist, GLint n){
  if(n <= 0) return;
  if(!glist->array){
    glist->nmax = ((n - 1) / glist->incr + 1) * glist->incr;
    glist->array = (char *)gl2psMalloc(glist->nmax * glist->size);
  }
  else
    if(n > glist->nmax){
      glist->nmax = ((n - 1) / glist->incr + 1) * glist->incr;
      glist->array = (char *)gl2psRealloc(glist->array,
					 glist->nmax * glist->size);
    }
}
...
GL2PSlist *gl2psListCreate(GLint n, GLint incr, GLint size){
  ...
  if(n < 0) n = 0;
  if(incr <= 0) incr = 1;
  glist = (GL2PSlist *)gl2psMalloc(sizeof(GL2PSlist));
  glist->nmax = 0;
  glist->incr = incr;
  glist->size = size;
  glist->n = 0;
  glist->array = NULL;
  gl2psListRealloc(glist, n);
  return(glist);
}

void gl2psListDelete(GL2PSlist *glist){
  gl2psFree(glist->array);
  gl2psFree(glist);
}

void gl2psListAdd(GL2PSlist *glist, void *data){
  glist->n++;
  gl2psListRealloc(glist, glist->n);
  memcpy(&glist->array[(glist->n - 1) * glist->size], data, glist->size);
}

GLint gl2psListNbr(GL2PSlist *glist){
  return(glist->n);
}

void *gl2psListPointer(GL2PSlist *glist, GLint index){
  if((index < 0) || (index >= glist->n))
    gl2psMsg(GL2PS_ERROR, "Wrong glist index in gl2psListPointer");
  return(&glist->array[index * glist->size]);
}

void gl2psListSort(GL2PSlist *glist,
		   int (*fcmp)(const void *a, const void *b)){
  qsort(glist->array, glist->n, glist->size, fcmp);
}

void gl2psListAction(GL2PSlist *glist, 
		     void (*action)(void *data, void *dummy)){
  GLint i, dummy;

  for(i = 0; i < gl2psListNbr(glist); i++)
    (*action)(gl2psListPointer(glist, i), &dummy);
}

void gl2psListActionInverse(GL2PSlist *glist, 
			    void (*action)(void *data, void *dummy)){
  GLint i, dummy;

  for(i = gl2psListNbr(glist); i > 0; i--)
    (*action)(gl2psListPointer(glist, i-1), &dummy);
}
...
void gl2psCutEdge(GL2PSvertex *a, GL2PSvertex *b, GL2PSplane plane, 
  ...
  c->rgba[0] = (1.0f-sect) * a->rgba[0] + sect * b->rgba[0];
  c->rgba[1] = (1.0f-sect) * a->rgba[1] + sect * b->rgba[1];
  c->rgba[2] = (1.0f-sect) * a->rgba[2] + sect * b->rgba[2];
  c->rgba[3] = (1.0f-sect) * a->rgba[3] + sect * b->rgba[3];
}
...
void gl2psAddPrimitiveInList(GL2PSprimitive *prim, GL2PSlist *glist){
  GL2PSprimitive *t1, *t2;

  if(prim->type != GL2PS_QUADRANGLE){
    gl2psListAdd(glist, &prim);
  }
  else{
    gl2psDivideQuad(prim, &t1, &t2);
    gl2psListAdd(glist, &t1);
    gl2psListAdd(glist, &t2);
...
void gl2psSplitPrimitive2D(GL2PSprimitive *prim, 
  ...
  /* glist of vertexs which will go in front and back Primitive */
  GL2PSvertex *front_list = NULL, *back_list = NULL;
  
  /* number of vertex in front and back glist */
...
void gl2psAddBoundaryInList(GL2PSprimitive *prim, GL2PSlist *glist){
  GL2PSprimitive *b;
  GLshort         i;
  GL2PSxyz        c;
...
void gl2psAddPolyPrimitive(GLshort type, GLshort numverts, 
  ...
  prim->boundary = static_cast<char>(boundary);
  prim->dash = static_cast<char>(dash);
  prim->width = width;
  prim->culled = 0;
  ...
  prim->verts[0].xyz[2] = static_cast<GLfloat>(
    prim->verts[0].xyz[2] - GL2PS_SIMPLE_OFFSET_LARGE);
  prim->verts[1].xyz[2] = static_cast<GLfloat>(
...
GLint gl2psGetVertex(GL2PSvertex *v, GLfloat *p){
  ...
  v->xyz[0] = p[0];
  v->xyz[1] = p[1];
  v->xyz[2] = static_cast<GLfloat>(GL2PS_DEPTH_FACT * p[2]);
...
void gl2psPrintPostScriptHeader(void){
  ...
  ctime_s(creationDate, sizeof(creationDate), &now);
  fprintf(gl2ps->stream, 
	  ...
	  "%%%%Pages: 1\n",
    gl2ps->title, GL2PS_VERSION, gl2ps->producer, creationDate);
...
void gl2psPrintTeXHeader(void){
  ...
  if(gl2ps->filename && strlen(gl2ps->filename) < 256){
    for(i = static_cast<int>(strlen(gl2ps->filename)) - 1; i >= 0; i--){
      if(gl2ps->filename[i] == '.'){
	strncpy_s(name, sizeof(name), gl2ps->filename, i);
	name[i] = '\0';
	break;
      }
    }
    if(i <= 0) strcpy_s(name, sizeof(name), gl2ps->filename);
  }
  else
...
GL2PSDLL_API void gl2psBeginPage(const char *title, const char *producer, 
  ...
  gl2ps->threshold[0] = 0.032f;
  gl2ps->threshold[1] = 0.017f;
  gl2ps->threshold[2] = 0.05f;
...
GL2PSDLL_API void gl2psText(const char *str, const char *fontname, GLint fontsize){
  ...
  prim->verts[0].xyz[2] = static_cast<GLfloat>(GL2PS_DEPTH_FACT) * pos[2];
  prim->depth = static_cast<GLfloat>(pos[2]);
  prim->culled = 0;
  prim->dash = 0;
  prim->width = 1;
  ...
  strcpy_s(prim->text->str, strlen(str) + 1, str); 
  prim->text->fontname = (char*)gl2psMalloc((strlen(fontname)+1)*sizeof(char));
  strcpy_s(prim->text->fontname, strlen(fontname) + 1, fontname);
...
GL2PSDLL_API void gl2psNumShadeColors(GLint nr, GLint ng, GLint nb){
  ...
  gl2ps->threshold[0] = nr ? 1.0f / static_cast<GLfloat>(nr) : 1.0f;
  gl2ps->threshold[1] = ng ? 1.0f / static_cast<GLfloat>(ng) : 1.0f;
  gl2ps->threshold[2] = nb ? 1.0f / static_cast<GLfloat>(nb) : 1.0f;
}
