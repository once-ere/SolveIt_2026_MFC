...
static png_uint_32
png_push_checked_u32(png_structp png_ptr, png_size_t size, const char* message)
{
...
png_push_crc_finish(png_structp png_ptr)
{
   if (png_ptr->skip_length && png_ptr->save_buffer_size)
   {
      ...
      png_ptr->skip_length -= png_push_checked_u32(png_ptr, save_size,
...
png_push_read_IDAT(png_structp png_ptr)
{
   ...
   if (png_ptr->idat_size && png_ptr->save_buffer_size)
   {
      ...
      png_ptr->idat_size -= png_push_checked_u32(png_ptr, save_size,
         "save_size overflowed in pngpread");
      png_ptr->buffer_size -= save_size;
      png_ptr->save_buffer_size -= save_size;
      png_ptr->save_buffer_ptr += save_size;
         ...
         if (!(png_ptr->zstream.avail_out) || ret == Z_STREAM_END)
         {
            if (text == NULL)
            {
               text = (png_charp)png_malloc(png_ptr,
                  png_push_checked_u32(png_ptr,
                     png_ptr->zbuf_size - png_ptr->zstream.avail_out +
                     key_size + 1, "text size overflowed in pngpread"));
               ...
               tmp = text;
                  text = (png_charp)png_malloc(png_ptr,
                    ...
                    text_size + png_ptr->zbuf_size - png_ptr->zstream.avail_out
                     + 1, "text size overflowed in pngpread"));
