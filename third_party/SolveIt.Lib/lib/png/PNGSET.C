...
static png_uint_32
png_set_checked_u32(png_structp png_ptr, png_size_t size, const char* message)
{
   if (size > (png_size_t)PNG_MAX_UINT)
      png_error(png_ptr, message);

   return (png_uint_32)size;
}
   ...
   png_charp units, png_charpp params)
{
   ...
   length = png_set_checked_u32(png_ptr, png_strlen(purpose) + 1,
      "pCAL purpose string is too large");
   ...
   length = png_set_checked_u32(png_ptr, png_strlen(units) + 1,
      "pCAL units string is too large");
   ...
   for (i = 0; i < nparams; i++)
   {
      length = png_set_checked_u32(png_ptr, png_strlen(params[i]) + 1,
         "pCAL parameter string is too large");
