...
static png_uint_32
png_write_checked_u32(png_structp png_ptr, png_size_t size, const char* message)
{
   ...
   png_size_t text_len)
{
   ...
   png_write_chunk_start(png_ptr, png_tEXt,
      png_write_checked_u32(png_ptr, key_len + text_len + 1,
...
#if !defined(PNG_NO_STDIO)
      char msg[50];
   snprintf(msg, sizeof(msg), "Unknown zTXt compression type %d", compression);
   ...
   png_int_32 X1, int type, int nparams, png_charp units, png_charpp params)
{
   ...
   for (i = 0; i < nparams; i++)
   {
      params_len[i] = png_write_checked_u32(png_ptr,
         png_strlen(params[i]) + (i == nparams - 1 ? 0 : 1),
         "pCAL parameter too large");
