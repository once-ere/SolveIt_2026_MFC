...
void qh_produce_output(void) {
  ...
  if (qh PRINTstatistics) {
    ...
    d_1= sizeof(setT) + (qh hull_dim - 1) * SETelemsize;
        fprintf(qh ferr, "\
        size in bytes: hashentry %zu merge %zu ridge %zu vertex %zu facet %zu\n\
          normal %d ridge vertices %d facet vertices or neighbors %zu\n",
...
coordT *qh_readpoints(int *numpoints, int *dimension, boolT *ismalloc) {
  ...
  if (qh CDDinput) {
	...
	while (isspace(*s))
    	...
    	strncat_s(qh rbox_command, sizeof (qh rbox_command), s, _TRUNCATE);
    }
    if (!s) {
