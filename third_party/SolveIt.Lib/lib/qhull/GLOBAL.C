...
void qh_init_qhull_command(int argc, char *argv[]) {
  ...
  if (argc) {
    if ((s= strrchr( argv[0], '\\'))) /* Borland gives full path */
      strcpy_s (qh qhull_command, sizeof(qh qhull_command), s+1);
    else
      strcpy_s (qh qhull_command, sizeof(qh qhull_command), argv[0]);
  ...
  for (i=1; i < argc; i++) {
    if (strlen (qh qhull_command) + strlen(argv[i]) + 1 < sizeof(qh qhull_command)) {
      strcat_s (qh qhull_command, sizeof(qh qhull_command), " ");
      strcat_s (qh qhull_command, sizeof(qh qhull_command), argv[i]);
    }else {
...
void qh_initflags(char *command) {
  ...
  if (command != &qh qhull_command[0]) {
    *qh qhull_command= '\0';
    strncpy_s(qh qhull_command, sizeof(qh qhull_command), command, _TRUNCATE);
  }
  ...
  while (*s) {
	...
	qh_strtod (s, &t);
      ...
      if (start < t) {
        ...
        memcpy(qh feasible_string, start, (size_t)(t-start));
        qh feasible_string[t-start]= '\0';
	...
	case 'O':
	  if (s[0] != ' ' || s[1] == '\"' || isspace (s[1])) {
	    ...
	    *t= '\0';
        ...
        if (freopen_s(&redirected, filename, "w", stdout) != 0 || !redirected) {
...
void qh_initqhull_start (FILE *infile, FILE *outfile, FILE *errfile) {
  ...
  memset((char *)&qh_qh, 0, sizeof(qhT));
#endif
  strcpy_s (qh qhull, sizeof(qh qhull), "qhull");
  qh_initstatistics();
  qh ANGLEmerge= True;
  qh DROPdim= -1;
  qh ferr= errfile;
...
void qh_option (char *option, int *i, realT *r) {
  ...
  sprintf_s (buf, sizeof(buf), "  %s", option);
  if (i)
    sprintf_s (buf+strlen(buf), sizeof(buf) - strlen(buf), " %d", *i);
  if (r)
    sprintf_s (buf+strlen(buf), sizeof(buf) - strlen(buf), " %2.2g", *r);
  len= (int)strlen(buf);
  qh qhull_optionlen += len;
  if (qh qhull_optionlen >= 80 && qh qhull_options[0]) {
    qh qhull_optionlen= len;
    strncat_s (qh qhull_options, sizeof(qh qhull_options), "\n", _TRUNCATE);
  }    
  strncat_s (qh qhull_options, sizeof(qh qhull_options), buf, _TRUNCATE);
} /* option */
