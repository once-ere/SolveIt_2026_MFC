...
// spewPrimitiveEPS
//*********************************
GLfloat *CPsRenderer::spewPrimitiveEPS(FILE * file, GLfloat * loc)
{
	int token;
	int nvertices, i;
	...
	case GL_COPY_PIXEL_TOKEN:
		{
			char buf[256];
			sprintf(buf, "Not implemented.  Unexpected token (%d).\n", token);
			throw buf;
		}
		break;
	...
	//	exit(1);
	char buf[256];
    sprintf(buf, "Incomplete implementation.  Unexpected token (%d).\n",
        token);
