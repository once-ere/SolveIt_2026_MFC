! $Id: trlcore.f90,v 1.11 2001/02/20 22:13:31 kewu Exp $
!!!
!\Description:
...
Subroutine trlanczos(op, info, nrow, mev, eval, evec, lde, base, ldb,&
  ...
  Nullify(alpha, beta, rr, rot, alfrot, betrot, lambda, res, yy, qa,&
	...
	!success = SETENVQQ("FORT6=CONOUT.txt")
	!success = SETENVQQ("FOR_TYPE=CONOUT.txt")
	!success = SETENVQQ("FOR_PRINT=CONOUT.txt")
...
Contains
  ...
  Subroutine log_error_state
    ...
    If (info%my_pe.Eq.0 .And. info%log_io.Ne.6) Then
       Write (97, *) 'TRLanczos returned with error ', info%stat
       If (info%verbose .Gt. 0) Then
          Write (97, *) 'Contents of most variables are dumped to ',&
...
!!$Print *, rr
  ...
  If (jnd.Lt.info%ntot .And. (tmp.Ge.wrk(1) .Or. wrk(1).Le.Tiny(tmp)&
       & .Or. jm1.Eq.kept)) Then
