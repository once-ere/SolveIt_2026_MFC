...
{
	return Lib3D::GetMeanArea(&m_ArrayFaceNeighbor);
}
...
{
	return Lib3D::GetMinArea(&m_ArrayFaceNeighbor);
}
...
CVertex3d *CVertex3d::GetNearestVertexNeighborWithFlag(int flag)
{
	...
	for(int i=0;i<size;i++)
	{
		...
		if(pVertex->GetFlag() == flag)
		{
			double tmp = Lib3D::DistanceSquare(this,pVertex);
			if(tmp < distance)
			{
...
int CVertex3d::NbSharpEdge(const double threshold)
{
	...
	if(IsOnBoundary())
		return 3;
	int NbSharp = Lib3D::NbSharpEdge(m_ArrayFaceNeighbor,threshold);
	//TRACE("NbSharp : %d\n",NbSharp);
	return NbSharp;
}
...
{
	return Lib3D::NormalSum(m_ArrayFaceNeighbor,pSum);
}
...
{
	return Lib3D::NormalMax(m_ArrayFaceNeighbor,pMax);
}
...
double CVertex3d::GetMeanCurveAround()
{
	...
	if(size == 0)
		return 0.0f;
	double sum;
	Lib3D::NormalSum(m_ArrayFaceNeighbor,&sum);
	return sum/size;
}
...
{
	double sum;
	if(Lib3D::NormalSum(m_ArrayFaceNeighbor,&sum))
		return sum;
	else
		return 0.0;
}
...
{
	double max;
	if(Lib3D::NormalMax(m_ArrayFaceNeighbor,&max))
		return max;
	else
	{
...
{
	double max;
	if(Lib3D::MaxAngleBetweenFaces(m_ArrayFaceNeighbor,&max))
		return max;
	else
		return 0.0;
}
...
{
	double max = 0.0;
	Lib3D::NormalMax(m_ArrayFaceNeighbor,&max);
	int size = m_ArrayVertexNeighbor.GetSize();
	for(int i=0;i<size;i++)
...
double CVertex3d::GetMeanLengthEdgeAround()
{
	...
	for(int i=0;i<size;i++)
		sum += Lib3D::Distance(m_ArrayVertexNeighbor[i],this);
