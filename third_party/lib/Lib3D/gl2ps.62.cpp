 ...
 *
 * $Id: gl2ps.c,v 1.48 2002/09/06 19:00:31 geuzaine Exp $
 *
...
GLvoid gl2psListRealloc(GL2PSlist *glist, GLint n){
  if(n <= 0) return;
  if(!glist->array){
...
GLvoid gl2psListDelete(GL2PSlist *glist){
  gl2psFree(glist->array);
  gl2psFree(glist);
}

GLvoid gl2psListAdd(GL2PSlist *glist, GLvoid *data){
  glist->n++;
...
GLvoid gl2psListActionInverse(GL2PSlist *glist, 
			      GLvoid (*action)(GLvoid *data, GLvoid *dummy)){
  GLint i, dummy;
...
GLvoid gl2psAddPrimitiveInList(GL2PSprimitive *prim, GL2PSlist *glist){
...
GLvoid gl2psAddBoundaryInList(GL2PSprimitive *prim, GL2PSlist *glist){
  GL2PSprimitive *b;
  GLshort         i;
  GL2PSxyz        c;
...
GL2PSDLL_API GLvoid gl2psBeginPage(char *title, char *producer, 
				   ...
				   GL2PSrgba *colormap, size_t buffersize, 
...
GL2PSDLL_API GLvoid gl2psText(const char *str, char *fontname, GLint fontsize){
  GLfloat         pos[4];
  GL2PSprimitive  *prim;
  GLboolean       valid;
