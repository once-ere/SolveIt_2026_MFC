...
extern "C" _stdcall	TRLAN77(void(*op)(int*, int*, double*, int*, double*, int*),
...
//	TCHAR buf[_MAX_PATH];
//	GetCurrentDirectory(_MAX_PATH, buf);
	QMStationary2Dtxt	= _bstr_t(g_szWorkDir.c_str());//buf
	QMStationary2Dtxt += L"\\QMStationary2D.txt";
	freopen( QMStationary2Dtxt, "w", stdout );
...
//			}
			...
			if ( GL_NO_ERROR !=  (e=glGetError()) )
			{
					//wglMakeCurrent(0, 0);
//					::ReleaseDC(pGLView->GetSafeHwnd(), hDC);
					throw SolveIt_Error("Unable to glGenLists", __FILE__, __LINE__);
//					ASSERT(0);
//					return;
			}
			//wglMakeCurrent(0, 0);
...
{
	stream_stdout = freopen( "stdout.txt", "w", stdout );
//	if (psi_xy)
//	{
//		delete[]psi_xy;
//		psi_xy = 0;
...
///////////////////////////////////////////////////////////////////////////////
	{
		_variant_t xit=long(j);
		_bstr_t bstr(xit);
		string s(bstr);
	glRasterPos2d(-4+0.4*j, 4);

	glListBase(2000);
	glCallLists(s.size(), GL_BYTE, (GLbyte *)s.c_str());
...
estring = gluErrorString(errorCode);
	MessageBox (0, (const char *) estring, "Nurbs Error:", MB_OK);
//exit (0);
}
///////////////////////////////////////////////////////////////////////////////
#define ON_ERROR_THROW(msg) \
	...
	{\
		glPopAttrib();\
		glPopMatrix();\
		throw SolveIt_Error(msg, __FILE__, __LINE__);\
	}
		//::ReleaseDC(pGLView->GetSafeHwnd(), hDC);
		//wglMakeCurrent(0, 0);
///////////////////////////////////////////////////////////////////////////////
#define ON_ERROR_RETURN \
	...
	{\
		...
		throw SolveIt_Error("CQMStationary2D", __FILE__, __LINE__);\
	}
		//::ReleaseDC(pGLView->GetSafeHwnd(), hDC)
		//wglMakeCurrent(0, 0);
...
//	double r = (_N2a)/(N);
	...
	//		::ReleaseDC(pGLView->m_hWnd, hDC);
			throw SolveIt_Error("fabs(amplitude) < 0.01e-4", __FILE__, __LINE__);
	//		return;
		}
	}
	else if (j == m_nNumFound)
	{
			throw SolveIt_Error("j == m_nNumFound", __FILE__, __LINE__);
...
//	glNewList(listEigenStates[j], GL_COMPILE_AND_EXECUTE);
	...
	if ( GL_NO_ERROR !=  (e=glGetError()) )
	{
		//wglMakeCurrent(0, 0);
	//	::ReleaseDC(pGLView->GetSafeHwnd(), hDC);
		throw SolveIt_Error("Unable to glNewList", __FILE__, __LINE__);
	//	ASSERT(0);
	//	return;
	}
...
try{
	...
	{
		throw SolveIt_Error("Unable to gluEndSurface(aNurbs)", __FILE__, __LINE__);
	//	throw SolveIt_Error("Unable to gluEndSurface(aNurbs)", THIS_FILE, __LINE__);
	}
	ON_ERROR_RETURN
