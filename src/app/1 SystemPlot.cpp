...
///////////////////////////////////////////////////////////////////////////////
	struct KEY{
		...
		_T("x"),		1,		KEY::DOUBLE,	(unsigned long) &x2,
		_T("x"),		0,		KEY::DOUBLE,	(unsigned long) &x1,
		_T("y"),		1,		KEY::DOUBLE,	(unsigned long) &y2,
		_T("y"),		0,		KEY::DOUBLE,	(unsigned long) &y1,
		_T("#pts"),		0,		KEY::LONG,		(unsigned long) &NDATA,
		_T("color"),	0,		KEY::LONG,		(unsigned long) &red,
		_T("color"),	1,		KEY::LONG,		(unsigned long) &green,
		_T("color"),	2,		KEY::LONG,		(unsigned long) &blue,
		_T("x-Label"),	0,		KEY::QSTRING,	(unsigned long) &xStr,
		_T("y-Label"),	0,		KEY::QSTRING,	(unsigned long) &yStr,
		0,0,KEY::UNDEFINED,0
	};
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
...
//	istrstream iss(buf, sizeof(buf)/sizeof(buf[0]));
//	char seps[]   = " ,\t\n";
	char seps[]   = ",\t\n";
	char *token=0;
///////////////////////////////////////////////////////////////////////////////
try
{
	...
	_tcsncpy(_options, Options, sizeof(_options)/sizeof(_options[0]));
	char*	c	=_options;
	char*	p	= options;
	bool bLiteral = false;
	do {
	...
	if (nc1 == 0) return false;
	if (nc2 != nc1) return false;
...
//	yStr	= const_cast<char*>(tree->GetyStr());
	xStr	= _tcsdup(m_xStr.c_str());
	yStr	= _tcsdup(m_yStr.c_str());
...
//get key
			--nc1;
			++j;
			if (!(isalpha(c[j]) || c[j] == '_')) return false;
			p = sbuf;
			do {
...
//Establish string and get the first token
			token = strtok( vbuf, seps );//_tcstok
			while( token != NULL )
			{
			
				for (int i=0; keys[i].sKey; ++i)
				{
					if (nTok == keys[i].nToken && !strcmp(sbuf, keys[i].sKey)) break;//_tcscmp
				}

				oss.seekp(0);
				if (keys[i].type == KEY::QSTRING) oss << token << ends;
				else
				{
					string out;
					string s(token);
...
top	= stackMachine->top;
	...
	{
		::MessageBox(0, except->name, "Error", MB_OK);
		stackMachine->terminate_cycle();
		return false;
	}
	catch (char * str)
			...
			//	iss.seekg(0);
				...
				switch(keys[i].type)
				{
					...
					case KEY::DOUBLE:
					{
						...
						if(!(iss>>x).fail())
						{
							*((double*)(keys[i].data)) = x;
						}
					}
					break;
					case KEY::QSTRING:
					{
						free((void *)*((char**)(keys[i].data)));
						*((char**)(keys[i].data)) = _tcsdup(buf);
					}
					break;
				}
...
//	RestoreState();Potential
	...
	if (!SetUserDefinedName(name, __External1)) return false;
...
//vector<DoThis>
	...
	if (data__QM_Stationary1D->m_pCQMStationary1D)
	{
		if (data__QM_Stationary1D->m_pCQMStationary1D->m_bStepCompleted)
		{
			data__QM_Stationary1D->m_pCQMStationary1D->DrawSystem();
		}
	}
///////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////
	if (player != NULL )
	{
		...
		glPushMatrix();
		{
			...
			oss << "birth rate = " << (float)player->g_Rate  << ends;
			string str(buf);
