...
*/
	...
	static PIXELFORMATDESCRIPTOR pfd = 
	{
				...
				static_cast<BYTE>(g_bpp),			// bpp
				0, 0, 0, 0, 0, 0,				// color bits ignored
				0,								// Don't use the alpha buffer
				0,								// shift bit ignored
				0,								// no accumulation buffer
				0, 0, 0, 0,						// accum bits ignored
				static_cast<BYTE>(g_z_buf),			// z-buffer
				0,								// no stencil buffer
...
// make the system font the device context's selected font 
	...
	else
	{
		...
		pFrame->PostMessage(WM_USER_Message, 3, (LPARAM) L"error GL_VERSION.");
	}
...
//1		glClear(GL_COLOR_BUFFER_BIT);
	...
	//	glMultMatrixd(&m[0][0]);
		...
		Vector3D axis	= tBall.axis();
		double angle	= -RAD_TO_DEG*tBall.angle();
