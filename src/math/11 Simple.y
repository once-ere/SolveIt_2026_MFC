...
%type	<p_counter>	functiondef mark
%type	<lval>	arglist
...
/*
functiondef :	DEFINE_USER_FUNCTION VAR	{ indefInArgList=indef=$2->indexInDataArray; ConvertToFunc( $2 );}
				'(' arglist ')'				{ ReserveStackMemory($4);indefInArgList= -1;}
				mark						{ instruction::nInstructionStart = $6;}
				'{' stmtlist '}'			{ gen_code( instruction::I_RETURN, 0 );CodeFunc( $2 );indef=-1;}
;
{ ReserveStackMemory(yyvsp[-5].lval);indefInArgList= -1;;
    break;}
case 8:
{ instruction::nInstructionStart = yyvsp[-3].p_counter;;
*/

functiondef :	DEFINE_USER_FUNCTION VAR	{
												indefInArgList=indef=$2->indexInDataArray;
												ConvertToFunc( $2 );
											}
				'(' arglist ')'				{
												ReserveStackMemory(yyvsp[-5].lval);
												indefInArgList= -1;
											}
				mark						{ instruction::nInstructionStart = yyvsp[-3].p_counter;}
				'{' stmtlist '}'			{ gen_code( instruction::I_RETURN, 0 );CodeFunc( $2 );indef=-1;}
;
...
mark:	  /* nothing */		{ $$ = next_free_spot_for_code_generation; }
	;
