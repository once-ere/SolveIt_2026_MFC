! $Id: trlan.f90,v 1.9 2001/09/06 17:13:38 kewu Exp $
...
!      OPEN (UNIT = 6, FILE = 'duh.txt')
  ...
  !
  If (info%maxlan < info%ned) Then
     Print *, 'info%maxlan (', info%maxlan, ') is smaller than ', &
          & 'info%ned (', info%ned, ') -- increase info%maxlan to ', &
          & info%ned+Max(info%ned,6)
     info%maxlan = info%ned + Max(info%ned, 6)
  End If
  If (info%maxlan < mev) Then
     If (mev < Max(100+info%ned, 10*info%ned)) Then
        Print *, 'info%maxlan (', info%maxlan, ') is smaller than ', &
             & 'mev (', mev, ') -- increase info%maxlan to ', mev
        info%maxlan = mev
     End If
  End If
...
!!!
! clear the counters inside info
!!!
Subroutine trl_clear_counter(info, nrow, mev, lde)
  Use trl_info
  Implicit None 
  ...
  End If
  info%maxlan = Min(Max(info%maxlan, mev-1), info%ntot)
  !!! clear regular counters
  info%tmv = -1
  info%klan = info%maxlan
