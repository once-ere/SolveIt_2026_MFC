...
C EPSH.  DABS(EPSH) = TOLERENCE FOR LEVELING.  IF EPSH IS
C NEGATIVE, REF CONTAINS THE INITIAL POINT SET.  DABS(EPSH)
...
C ORDER POLYNOMIAL OF DEGREE 30.
      ...
      DOUBLE PRECISION EPSH, HMAX, X, Y, H, A, PI, Q, S, X1, XA, XE, DCOS
      INTEGER N, M, K, MAXIT, REF, I, J, P, Q1, Q2, R, K0, K1, B1,
     * B2, Z, ITEMP, MOLD, HIGH, JJ, II, LOW, EQUAL, NTAPE, B3
C MACHINE DEPENDENT CONSTANTS.
C NTAPE IS THE PRINTER UNIT NUMBER.'CONOUT.txt'
...
C DECLARATION TO DOUBLE PRECISION AND ABS TO DABS.
     ...
     * HIGH, EQUAL, II
      DOUBLE PRECISION EPSH, H, HZ1, HZP, MAX, MAXL, MAXR, SIG
      OPEN (UNIT = 6, FILE = WORK_DIR, SHARE='DENYNONE')
      EQUAL = 0
      L = 0
      ITEMP = Z(2)
...
C SIGN ASSUMED FOR H(I).
   ...
   60   CONTINUE
        ...
        ITEMP = Z(ITEMP)
        MAXL = DABS(MAX)
        IF (DABS(MAX-H(ITEMP))/MAXL-EPSH) 80, 80, 70
...
C IMMEDIATLY TERMINATES THE PROCESS.
  390 DO 410 I=2,P
        ITEMP = Z(I)
        IF (DABS(H(INDL))-DABS(H(ITEMP))) 310, 400, 400
  400   J = Z(I)
        Z(I) = INDL
        INDL = INDEX
        INDEX = J
...
C IS GREATER IN MAGNITUDE THAN THE PROSPECTIVE REPLACEE.
  440 DO 460 II=2,P
        ...
        ITEMP = I + 1
        HIGH = Z(ITEMP)
        IF (DABS(H(INDR)-DABS(H(HIGH)))) 310, 450, 450
  450   J = Z(ITEMP)
        Z(ITEMP) = INDR
        INDR = INDEX
        INDEX = J
